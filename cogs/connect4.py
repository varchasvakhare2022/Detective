...
class Connect4(commands.Cog):
    ...
    async def start_invite(self, interaction: Interaction) -> None:
        await interaction.response.send_message('Started a session for you, follow below instructions -', ephemeral=True)
        message = await interaction.channel.send(
            f'{interaction.user.display_name} wants to start a game of Connect 4\n'
            f'Waiting for {interaction.user.display_name} to pick a color!'
        )
        self.waiting_games[message.id] = (message, interaction.user, None)
        for emoji in TOKENS:
    ...
    async def finish_game(self, game, message, result):
        ...
        if result == game.DRAW:
            ...
            await message.channel.send(f"{game.player1.mention} has won the game")
        elif result == game.PLAYER2_WINNER:
            footer = f'{game.player2.display_name} has won the game'
            await message.channel.send(f"{game.player2.mention} has won the game")
    ...
    async def connect4(self, interaction: Interaction):
        """Start a game of Connect 4"""
        await self.start_invite(interaction)
